...
c       slow Z variation(except at very small Z)
      if(xh+za(mfm) .gt. 1.) mfm=m3
        if(xh+za(mfm) .gt. 1.) then
            if(m1 .le. 1) then
              write(0,'("special case: X,Z location not covered by"
     x                 ," logic")')
              stop
            endif
          m1=m1-1
...
c__________
  ...
  123 continue
     ...
     x    (zz(mh,mzin) .ne. zz(mf,mzin))) then
      write(0,'("Z does not match Z in GN93hz files you are"
     x ," using")')
      stop
      endif
...
c
   ...
   61 write(0,'(" Mass fractions exceed unity")')
c                  with a return
      opaerr = 61
      return
   62 write(0,'(" T6/LogR outside of table range")')
c     write(0,'("slt,alt(1),alt(nt),slr,alr(1),alr(nre),l3s,i,k3s,
c    x nta(i+1)",6e12.5,4i5)') slt,alt(1),alt(nt),slr,alr(1),
c    x alr(nre),l3s,i,k3s,nta(i+1)
c                  with a return
      opaerr = 62
      return
   64 write(0,'(" X not equal to zero: To run this case it
     .is necessary"/ "to recompile with parameter (mx=1)")')
      stop
   65 write(0,'("Too few R values; NRE+1-NRB < 6")')
      opaerr = 65
      return
   66 write(0,'(" Z does not match Z in codata* files you are",
     . " using")')
      opaerr = 66
      stop
      end
...
c.....        Dlog(k)/Dlog(R) smoothed in both log(T6) and Log(R).
      ...
      dopactd=dopact-3.*dopacr
        if (opact .gt. 1.e+15) then
          write(0,'("Interpolation indices out of range",
...
c      read(2,'(f10.5)') dum
     ...
     .itab(m,i),x(m,i),y(m,i),zz(m,i)
      ...
      read (2, *)
      read (2, *)
      write (0, '("m = ",i0,", n(m) = ",i0,", i = ", i0)') m, n(m), i
      read(2,'(4x,19f7.1)') (alrf(kk),kk=1,nrm)
      read (2, *)
        do k=1,ntm
